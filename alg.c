 ...
 */
void alg_draw_red_location(struct coord *cent, struct images *imgs,
    struct image_data *imgdata, int style, int mode, int process_thisframe) {
    int x, y;
...
int alg_diff_standard(struct context *cnt, unsigned char *new)
{
    ...
    if (!mask && !smartmask_speed) {
        ...
        for (; i > 0; i--) {
            ...
            if (curdiff > noise) {
                /* Increase smart_mask sensitivity every frame when motion is detected.
                   With speed=5, mask is increased by 1 every second.  To be able to increase
                   by 5 every second (with speed=10) we add 5 here. NOT related to the 5 at
                   ratio-calculation. */
                if (cnt->event_nr != cnt->prev_event) {
                    (*smartmask_buffer) += SMARTMASK_SENSITIVITY_INCR;
                }
