...
/* In the top section are the functions that are used when processing the RTSP
camera feed.  Since these functions are internal to the RTSP module, and many
require FFmpeg structures in their declarations, they are within the HAVE_FFMPEG
block that eliminates them entirely when FFmpeg is not present.

The functions netcam_setup_rtsp, netcam_connect_rtsp, netcam_shutdown_rtsp, and
netcam_next_rtsp are called from netcam.c and therefore must be defined even if
FFmpeg is not present.  They must also not have FFmpeg structures in the
declarations.  Simple error messages are raised if called when no FFmpeg is
found. */
...
/* netcam_check_pixfmt: Determine whether pix_format is YUV420P */
int netcam_check_pixfmt(netcam_context_ptr netcam) {
    int retcd = -1;

    if ((netcam->rtsp->codec_context->pix_fmt == MY_PIX_FMT_YUV420P) ||
        (netcam->rtsp->codec_context->pix_fmt == MY_PIX_FMT_YUVJ420P)) {
        retcd = 0;
    }
...
static void netcam_buffsize_rtsp(netcam_buff_ptr buff, size_t numbytes) {
    ...
    MOTION_LOG(DBG, TYPE_NETCAM, NO_ERRNO,
        "%s: expanding buffer from [%d/%d] to [%d/%d] bytes.",
...
/* decode_packet
This routine takes in the packet from the read and decodes it into the frame.
It then takes the frame and copies it into the netcam buffer.
...
Returns the size of the decoded frame, or 0 on failure. */
static int decode_packet(AVPacket *packet, netcam_buff_ptr buffer, AVFrame *frame,
    AVCodecContext *cc) {
    int check = 0;
    int frame_size = 0;
    ...
    avpicture_layout((const AVPicture *)frame, cc->pix_fmt, cc->width,
        cc->height, (unsigned char *)buffer->ptr, frame_size);
...
/* open_codec_context: This routine opens the codec context for the indicated
stream.
...
Returns 0 on success, or the error code from FFmpeg (which is negative), or -1
if the FFmpeg method failed without setting an error code. */
static int open_codec_context(int *stream_idx, AVFormatContext *fmt_ctx,
    ...
    if ((ret = av_find_best_stream(fmt_ctx, type, -1, -1, NULL, 0)) < 0) {
		...
		MOTION_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
            "%s: Could not find stream in input!: %s", errstr);
        return ret;
    }
    ...
    *stream_idx = ret;
    ...
    if (ret < 0) {
        av_strerror(ret, errstr, sizeof(errstr));
        MOTION_LOG(ERR, TYPE_NETCAM, NO_ERRNO, "%s: Failed to open codec: %s",
            errstr);
        return ret;
    }
...
/* netcam_interrupt_rtsp: This function is called during the FFmpeg blocking
functions.  These include the opening of the format context as well as the
reading of the packets from the stream.  Since this is called during all
blocking functions, the process uses the readingframe flag to determine whether
to timeout the process.

ctx     We pass in the rtsp context to use it to look for the readingframe flag
        as well as the time that we started the read attempt.

Returns 0 on success, or -1 on failure (which interrupts the caller). */
...
/* netcam_read_rtsp_image: This function reads the packet from the camera.  It
is called extensively, so only absolutely essential functions and allocations
are performed.
...
int netcam_read_rtsp_image(netcam_context_ptr netcam) {
    ...
    /* Fail fast if we got a bad context. */
    if (netcam->rtsp == NULL) {
        return -1;
    }
    ...
    while ((size_decoded == 0) &&
        (av_read_frame(netcam->rtsp->format_context, &packet) >= 0)) {
        ...
        size_decoded = decode_packet(&packet, buffer, netcam->rtsp->frame,
            netcam->rtsp->codec_context);
    ...
    if (size_decoded == 0) {
        // Something went wrong.  End of stream?  Were we interrupted?
        netcam_rtsp_close_context(netcam);
        return -1;
    }

    /* Calculate our "running average" time for this netcam's frame
    transmissions (except for the first time).  Note that the average frame time
    is held in microseconds. */
    ...
    /* The read is complete.  Set the current receiving buffer atomically as
    latest, make the previous latest buffer the new receiving, and signal
    pic_ready. */
    ...
    /* We have a new frame ready.  We send a signal so that any thread (e.g.
    the motion main loop) waiting for the next frame to become available may
    proceed. */
...
/* netcam_rtsp_resize_ntc: This function notifies the user of the need to
transcode the netcam image (which uses a lot of CPU resources).
...
/* netcam_rtsp_open_context: This function opens the format context for the
camera.
...
int netcam_rtsp_open_context(netcam_context_ptr netcam) {
    ...
    if (netcam->rtsp->path == NULL) {
        if (netcam->rtsp->status == RTSP_NOTCONNECTED) {
            MOTION_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                "%s: Null path passed to connect (%s)", netcam->rtsp->path);
        }
        return -1;
    }
    ...
    retcd = open_codec_context(&netcam->rtsp->video_stream_index, netcam->rtsp->format_context, AVMEDIA_TYPE_VIDEO);
    if (retcd < 0) {
        if (netcam->rtsp->status == RTSP_NOTCONNECTED){
            av_strerror(retcd, errstr, sizeof(errstr));
...
/* netcam_rtsp_open_sws: This function opens the rescaling context components.
...
int netcam_rtsp_open_sws(netcam_context_ptr netcam) {
    ...
    /* The scaling context is used to change dimensions to config file and also
    if the format sent by the camera is not YUV420. */
    netcam->rtsp->swsctx = sws_getContext(netcam->rtsp->codec_context->width,
        netcam->rtsp->codec_context->height,
        netcam->rtsp->codec_context->pix_fmt, netcam->width, netcam->height,
        MY_PIX_FMT_YUV420P, SWS_BICUBIC, NULL, NULL, NULL);
    if (netcam->rtsp->swsctx == NULL) {
        if (netcam->rtsp->status == RTSP_NOTCONNECTED) {
            MOTION_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                "%s: unable to allocate scaling context.  Fatal error.  Check FFmpeg/Libav configuration");
        }
        netcam_rtsp_close_context(netcam);
        return -1;
    }

    netcam->rtsp->swsframe_size = avpicture_get_size(MY_PIX_FMT_YUV420P,
        netcam->width, netcam->height);
    if (netcam->rtsp->swsframe_size <= 0) {
        if (netcam->rtsp->status == RTSP_NOTCONNECTED){
            MOTION_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                "%s: Error determining size of frame out");
        }
        netcam_rtsp_close_context(netcam);
        return -1;
    }
...
/* netcam_rtsp_resize: This function reencodes the image to yuv420p with the
desired size.
...
int netcam_rtsp_resize(unsigned char *image, netcam_context_ptr netcam) {
    ...
    retcd = avpicture_fill((AVPicture *)netcam->rtsp->swsframe_in,
        (uint8_t *)netcam->latest->ptr, netcam->rtsp->codec_context->pix_fmt,
    ...
    if (retcd < 0) {
        if (netcam->rtsp->status == RTSP_NOTCONNECTED) {
            av_strerror(retcd, errstr, sizeof(errstr));
            MOTION_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                "%s: Error allocating picture in: %s", errstr);
        }
        netcam_rtsp_close_context(netcam);
        return -1;
    }

    buffer_out = av_malloc(netcam->rtsp->swsframe_size * sizeof (uint8_t));
    retcd = avpicture_fill((AVPicture *)netcam->rtsp->swsframe_out, buffer_out,
        MY_PIX_FMT_YUV420P, netcam->width, netcam->height);
    if (retcd < 0) {
        if (netcam->rtsp->status == RTSP_NOTCONNECTED) {
            av_strerror(retcd, errstr, sizeof(errstr));
            MOTION_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                "%s: Error allocating picture out: %s", errstr);
        }
        netcam_rtsp_close_context(netcam);
        return -1;
    }

    retcd = sws_scale(netcam->rtsp->swsctx,
        (const uint8_t* const *)netcam->rtsp->swsframe_in->data,
        netcam->rtsp->swsframe_in->linesize, 0,
        netcam->rtsp->codec_context->height, netcam->rtsp->swsframe_out->data,
        netcam->rtsp->swsframe_out->linesize);
    if (retcd < 0) {
        if (netcam->rtsp->status == RTSP_NOTCONNECTED) {
            av_strerror(retcd, errstr, sizeof(errstr));
            MOTION_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                "%s: Error resizing/reformatting: %s", errstr);
        }
        netcam_rtsp_close_context(netcam);
        return -1;
    }

    retcd = avpicture_layout((const AVPicture *)netcam->rtsp->swsframe_out,
        MY_PIX_FMT_YUV420P, netcam->width, netcam->height,
        (unsigned char *)image, netcam->rtsp->swsframe_size);
    if (retcd < 0) {
        if (netcam->rtsp->status == RTSP_NOTCONNECTED){
            av_strerror(retcd, errstr, sizeof(errstr));
            MOTION_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                "%s: Error putting frame into output buffer: %s", errstr);
        }
        netcam_rtsp_close_context(netcam);
        return -1;
    }
...
/* Functions which do not depend upon FFmpeg follow. */

/* netcam_connect_rtsp: This function initiates the connection to the rtsp
camera.
...
/* netcam_shutdown_rtsp: This function closes and frees all the items for rtsp.
...
/* netcam_setup_rtsp: This function sets up all the necessary items for the rtsp
camera.
...
int netcam_setup_rtsp(netcam_context_ptr netcam, struct url_t *url) {
    ...
    if (netcam->rtsp == NULL) {
        MOTION_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
            "%s: unable to create rtsp context");
        netcam_shutdown_rtsp(netcam);
        return -1;
    }

    /* Allocate space for a working string to contain the path.  The extra 5 is
    for "://", ":" and string terminator. */
    ...
    /* Need a method to query the path and determine the authentication type. */
    ...
    /* Documentation does not indicate thread safety on these init functions, so
    we lock them just in case.  */
    pthread_mutex_lock(&global_lock);
    av_register_all();
    avcodec_register_all();
    pthread_mutex_unlock(&global_lock);

    /* The RTSP context should be all ready to attempt a connection with the
    server, so we try... */
...
/* netcam_next_rtsp: This function moves the picture to the image buffer.  If
the picture is not in the correct format for size it will put it into the
requested format.
